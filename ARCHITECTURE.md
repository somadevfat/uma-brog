...
### 3.2. Full-stack Type Safety via Hono RPC

フロントエンドと API 間の通信には、必ず **Hono RPC (`hc`)** を活用します。API ルートで定義された型をフロントエンドで直接利用することで、パスや型情報の乖離（「型エラー」や「URLの打ち間違い」）をゼロにします。URL 文字列の直接記述による脆弱な通信は原則禁止します。
...
### 3.6. Biome によるコード品質の維持 (Fast Tooling)
